...
itrunc(struct inode *ip)
{
  ...
  for(i = 0; i < NDIRECT; i++){
    if(ip->addrs[i]){
      ip_addrs = (ip->addrs[i]&0x00FFFFFF);
      bfree(ip->dev, ip_addrs);
      ip->addrs[i] = 0;
    }
  }
  
  if(ip->addrs[NDIRECT]){
    ip_addrs = (ip->addrs[NDIRECT]&0x00FFFFFF);
    bp = bread(ip->dev, ip_addrs);
    a = (uint*)bp->data;
    for(j = 0; j < NINDIRECT; j++){
      if(a[j]) bfree(ip->dev, (a[j]&0x00FFFFFF));
    }
    brelse(bp);
    ip_addrs = (ip->addrs[NDIRECT]&0x00FFFFFF);
    bfree(ip->dev, ip_addrs);
...
stati(struct inode *ip, struct stat *st)
{
  ...
  if(ip->type == T_CHECKED) {
      for(i = 0; i < (NDIRECT + 1); i++) {
        tmp = 0;
        if(i != NDIRECT) {
          store = ip->addrs[i]&0xFF000000;
          store = (store >> 24);
          checksum = checksum^(uchar)(store);

        } else {
          /* Handling indirect pointers */
          struct buf *bp;
          bp = bread(ip->dev, ip->addrs[NDIRECT]); 
          uint *ptr = (uint *)bp->data;
          int num = BSIZE/sizeof(uint);
          for(j = 0; j < num; j ++) {
            store = ptr[j]&0xFF000000;
            store = (store >> 24);
            tmp = tmp ^ (uchar)store;
          }
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  /* Making sure the size does not exceed BSIZE */
  int i, size;
  if(n > BSIZE) size = BSIZE;
  else size = n;
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    if(ip->type == T_CHECKED) {
      struct buf *bp2;
      uchar store = 0;
      int divide = off/BSIZE;
      if((divide) < NDIRECT) check_1 = ((ip->addrs[divide]&0xFF000000) >> 24);
      divide = divide - NDIRECT;
      if((divide) < NINDIRECT) {
        bp2 =(bread(ip->dev, ip->addrs[NDIRECT]));
        uint *ptr = (uint *)bp2->data;
        check_1 = ((ptr[divide]&0xFF000000) >> 24);
        brelse(bp2);
      }
      for(i = 0; i < size; i ++) store = store^bp->data[i];
      if(check_1 != (uint)store) {
        brelse(bp);
        return -1;
      }
    }
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    ...
    uchar tmp = 0;
    if(ip->type == T_CHECKED) {
      for(i = 0; i < size; i ++) tmp = tmp^bp->data[i];
      check = (((uint)tmp) << 24);
      int divide = off/BSIZE;
      if(divide < NDIRECT) {
        ip->addrs[divide] = (ip->addrs[divide]&0x00FFFFFF)|check;
      }
      divide = divide - NDIRECT;
      if(divide < NINDIRECT) {
        bp2 = bread(ip->dev, ip->addrs[NDIRECT]);
        uint *ptr = (uint *)bp2->data;
        ptr[divide] = (ptr[divide]&0x00FFFFFF)|check;
        bwrite(bp2);
        brelse(bp2);
      }
    }
