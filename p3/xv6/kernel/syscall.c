...
{
  // cprintf("in fetch int");
  //cprintf("p->sz : %d\n", p->sz);
  /* Make sure address is not in between the heap and stack */
  if((addr >= p->sz && addr < p->stackaddr) || (addr+4 > p->sz && addr+4 < p->stackaddr))
	return -1;
  *ip = *(int*)(addr);
  return 0;
}
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  if(((uint)i >= proc->sz && (uint)i < proc->stackaddr) || ((uint)i+size > proc->sz && (uint)i < proc->stackaddr) || (uint)i < PGSIZE)
    return -1;
  *pp = (char*)i;
  return 0;
}
